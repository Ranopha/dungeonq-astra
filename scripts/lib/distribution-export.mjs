...
import { hasReleaseSecret, isPublicSourcePath, isPythonCachePath } from './release-paths.mjs';
...
export async function exportDistribution({ root, output, target }) {
  ...
  async function copy(path, destination = path) {
    // Amazon does not ship the Astra presentation site or its site-only import test.
    if (target === 'amazon' && destination === 'tests/astra-site-runtime.test.mjs') return;
    ...
    if (stat.isDirectory()) {
      for (const name of (await readdir(source)).sort()) {
        const next = posix.join(destination, name);
        // Python clients may have been tested locally. Their generated bytecode is never release input.
        if (!isPythonCachePath(next)) await copy(posix.join(path, name), next);
      }
      return;
    }
