...
for (let index = 0; index < args.length; index += 2) {
  if (!['--data-dir', '--seed', '--depth', '--web-port', '--actor-port', '--mcp-port'].includes(args[index])
    || !args[index + 1] || options.has(args[index])) throw new Error('Use npm run defense -- [--data-dir private-directory] [--seed 42] [--depth 4] [--web-port 4196] [--actor-port 4197] [--mcp-port 4198]');
